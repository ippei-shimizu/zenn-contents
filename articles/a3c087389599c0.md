...
```rb:app/controllers/api/fruits/orders_controllers.rb
...
#### ◼︎ 注文に関する責務が混在したアクションが同じクラスに定義されている。
 ...
 `current_order_summary` `past_orders` `search`はデータ取得ロジック・データ加工・レスポンス生成の責務を持っています。そのため、単一責務の原則に則っておらず、Fat Controllerの原因になっています。
...
上記のようにクラスを分割することにより、`app/controllers/api/fruits/orders_controllers.rb`では以下のように呼び出すだけになります。
...
:::details app/controllers/api/fruits/orders_controllers.rbの全体コード
